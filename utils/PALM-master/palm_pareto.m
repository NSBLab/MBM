...
if any(Pidx),
    ...
    nP   = size(Gdist,1);
    if rev,
        [~,Gdist,Gcdf] = palm_competitive(Gdist,'descend',true); %Trang changed Gdist to Gdist(:) to deal with err
    else
        [~,Gdist,Gcdf] = palm_competitive(Gdist,'ascend',true); %Trang changed Gdist to Gdist(:) to deal with err
    end
    Gcdf = Gcdf/nP;
